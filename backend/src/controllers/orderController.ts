...
/**
 * Lấy hoặc tạo khách hàng "Khách ngoài hệ thống" dùng cho đơn ngoài hệ thống
 */
const getOrCreateGuestCustomer = async (): Promise<string> => {
  let guest = await prisma.customer.findFirst({
    where: { code: 'EXTERNAL_GUEST' }
  });
  if (!guest) {
    guest = await prisma.customer.create({
      data: {
        code: 'EXTERNAL_GUEST',
        name: 'Khách ngoài hệ thống',
        phone: '0000000000',
        address: 'Địa chỉ nhập khi tạo đơn'
      }
    });
  }
  return guest.id;
...
/**
 * Lấy hoặc tạo sản phẩm hệ thống cho đơn ngoài (nhập tay tên/trọng lượng/giá)
 */
const getOrCreateExternalProduct = async (productName: string, unitPrice: number): Promise<string> => {
  let prod = await prisma.product.findFirst({
    where: { code: 'SP_EXTERNAL' }
  });
  if (!prod) {
    prod = await prisma.product.create({
      data: {
        code: 'SP_EXTERNAL',
        name: 'Sản phẩm ngoài hệ thống',
        listPriceNet: 0,
        minSellPriceNet: 0,
        unit: 'Cái'
      }
...
/**
 * Tạo đơn hàng ngoài hệ thống - KHÔNG lấy dữ liệu nội bộ, chỉ form nhập + gửi VTP
 * body: productName, productQuantity, productWeight(g), productPrice(COD), receiver*, pushToVTP?
 */
export const createOrderOutsideSystem = async (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
    const {
      productName,
    ...
    const weight = Math.max(100, parseInt(String(productWeight)) || 500);
    const price = Math.max(0, parseFloat(String(productPrice)) || 0);
    if (!receiverName || !receiverPhone || !receiverAddress) {
      return res.status(400).json({ message: 'Vui lòng nhập đầy đủ thông tin người nhận' });
    }
    if (!receiverProvince || !receiverDistrict || !receiverWard) {
      return res.status(400).json({ message: 'Vui lòng chọn địa chỉ (Tỉnh/Quận/Phường)' });
    }
    const rProvinceId = receiverProvinceId != null ? parseInt(String(receiverProvinceId), 10) : NaN;
    const rDistrictId = receiverDistrictId != null ? parseInt(String(receiverDistrictId), 10) : NaN;
    const rWardId = receiverWardId != null ? parseInt(String(receiverWardId), 10) : NaN;
    if (pushToVTP && (isNaN(rProvinceId) || isNaN(rDistrictId) || isNaN(rWardId))) {
      return res.status(400).json({ message: 'Khi đẩy VTP cần gửi ID địa chỉ (receiverProvinceId, receiverDistrictId, receiverWardId)' });
    }

    const customerId = await getOrCreateGuestCustomer();
    const productId = await getOrCreateExternalProduct(productName, price);
    const totalAmount = price * qty;
    const count = await prisma.order.count();
    const code = `DH-${String(count + 1).padStart(6, '0')}`;
    const orderDate = new Date();
    const extNote = note ? `${note}\n[VTP_EXT]${JSON.stringify({ productName, productWeight: weight })}` : `[VTP_EXT]${JSON.stringify({ productName, productWeight: weight })}`;
    ...
    const order = await prisma.order.create({
      data: {
        ...
        paymentStatus: 'PENDING',
        orderStatus: pushToVTP ? 'CONFIRMED' : 'DRAFT',
        shippingStatus: pushToVTP ? 'CONFIRMED' : 'PENDING',
        confirmedAt: pushToVTP ? new Date() : null,
        confirmedById: pushToVTP ? user.id : null,
        note: extNote,
        receiverName,
        receiverPhone,
        receiverAddress,
        ...
        receiverDistrict,
        receiverWard,
        receiverProvinceId: isNaN(rProvinceId) ? null : rProvinceId,
        receiverDistrictId: isNaN(rDistrictId) ? null : rDistrictId,
        receiverWardId: isNaN(rWardId) ? null : rWardId,
        items: {
          create: [{
            code: `${code}-1`,
            productId,
    ...
    await logAudit({
      ...
      objectId: order.id,
      result: 'SUCCESS',
      details: `Created outside-system order ${code}`,
      req
    });
    ...
    if (pushToVTP) {
      try {
        ...
        const ext = extMatch ? (() => { try { return JSON.parse(extMatch[1]); } catch { return {}; } })() : {};
        const pName = ext.productName || productName || 'Sản phẩm';
        const pWeight = ext.productWeight || weight || 500;
        const sProvinceId = parseInt(process.env.VTP_SENDER_PROVINCE || '1', 10);
        const sDistrictId = parseInt(process.env.VTP_SENDER_DISTRICT || '8', 10);
        const sWardId = parseInt(process.env.VTP_SENDER_WARD || '175', 10);
        const totalShipWeight = Math.max(100, Math.round(pWeight * qty));
        const result = await viettelPostService.createOrder({
          orderId: order.code,
          orderDate: order.orderDate,
          senderName: process.env.VTP_SENDER_NAME || 'KAGRI BIO',
          senderPhone: process.env.VTP_SENDER_PHONE || process.env.VTP_USERNAME || '0352737467',
          senderAddress: process.env.VTP_SENDER_ADDRESS || 'Số 103, ngõ 95, Đạo Xuyên, Bát Tràng',
          ...
          productName: String(pName).substring(0, 100),
          productQuantity: qty,
          productWeight: totalShipWeight,
          productPrice: Number(order.finalAmount),
          moneyCollection: Number(order.finalAmount),
          note: note || ''
        });
        await prisma.order.update({
          where: { id_orderDate: { id: order.id, orderDate: order.orderDate } },
          data: {
            shippingStatus: 'SHIPPING',
            shippingProvider: 'VIETTEL_POST',
            trackingNumber: result.trackingNumber,
            shippingCode: result.trackingNumber,
            shippedAt: new Date()
          }
        });
        ...
        return res.status(201).json({ ...updated, vtpTrackingNumber: result.trackingNumber, message: 'Đã tạo đơn và đẩy lên Viettel Post thành công' });
      } catch (vtpErr: any) {
        const errMsg = vtpErr?.response?.data?.message || vtpErr?.message || 'Lỗi đẩy VTP';
        console.error('Push VTP on create outside failed:', vtpErr);
        return res.status(201).json({
          ...order,
          message: `Đã tạo đơn nhưng đẩy VTP thất bại: ${errMsg}. Bạn có thể thử đẩy lại từ danh sách đơn hàng.`
        });
      }
    }
    ...
    console.error('Create order outside system error:', error);
    const msg = error?.message || error?.meta?.cause || 'Lỗi khi tạo đơn hàng';
