...
1. Tạo đơn:
  ...
  - **Tiền thu hộ (COD) Viettel Post:** `POST /api/orders` nhận tùy chọn `depositAmount` (đã cọc) và `shippingFee` (phí VC thu tại shop, lưu `orders.shipping_fee`). `orders.final_amount` = tổng hàng − giảm giá (không trừ cọc). `orders.cod_amount` = max(0, `final_amount` − `deposit_amount`) — **phí vận chuyển thu shop không cộng vào COD**. Khi đẩy VTP (`push-viettel-post`), `MONEY_COLLECTION` = `cod_amount` nếu có, ngược lại `final_amount` (đơn cũ).
