...
Truy cập route `/system` (SPA): Ưu tiên có menu **Hệ thống** (`/system`); nếu thiếu menu nhưng có một trong các quyền nghiệp vụ trên, `PermissionRoute` vẫn cho cho phép vào. Tổng cộng **92 quyền** trong catalog (tại `backend/src/constants/permissionsCatalog.ts`), đồng bộ với DB qua `ensureDefaultPermissionsCatalog` khi khởi động backend.
...
- **Danh sách 78 quyền trong catalog** (15 nhóm theo module, `backend/src/constants/permissionsCatalog.ts` — `DEFAULT_PERMISSIONS`; UI: `PERMISSION_GROUPS` trong `RoleGroupManager.tsx`):
  1. **Hệ thống (10):** `FULL_ACCESS`, `MANAGE_SYSTEM`, `VIEW_LOGS`, `VIEW_ROLE_GROUPS`, `MANAGE_ROLE_GROUPS`, `MANAGE_EMPLOYEE_ACCOUNTS`, `VIEW_SETTINGS`, `EDIT_SETTINGS`, `STAFF_LOGOUT`, `STAFF_LOCK`
