...
- Chi phí campaign:
  - `GET/POST/PUT/DELETE /api/marketing/campaigns/:campaignId/costs` — **bắt buộc** gắn chi phí với chiến dịch qua URL; nền tảng (`platform`), metric, mô tả, chứng từ **không** bắt buộc (đồng bộ FE `MarketingCostEffectiveness`).
...
- **Prisma P3009 (migration thất bại trên DB, `migrate deploy` dừng):** một migration đã **bắt đầu** nhưng **không hoàn tất** (ghi trong `_prisma_migrations` với `finished_at` null hoặc trạng thái failed). **Trước khi sửa:** `npm run backup:db` trong `backend/` (hoặc `pg_dump` tương đương). **Bước 1 — kiểm tra DB:** kết nối Postgres (`psql` hoặc client), xem bản ghi migration lỗi: `SELECT migration_name, started_at, finished_at, logs FROM "_prisma_migrations" ORDER BY started_at DESC LIMIT 5;`. **Bước 2 — so với SQL trong repo:** mở `backend/prisma/migrations/<tên_thư_mục>/migration.sql` và xác nhận schema thực tế (bảng/cột/index/constraint đã có chưa). **Bước 3 — xử lý:** (a) Nếu thay đổi trong migration **đã áp dụng đủ** và khớp schema hiện tại: `npx prisma migrate resolve --applied "<tên_thư_mục_migration>"` trong `/app/backend` với đúng `DATABASE_URL`. (b) Nếu migration **thất bại và transaction đã rollback** (DB không có thay đổi mong muốn): `npx prisma migrate resolve --rolled-back "<tên_thư_mục_migration>"`, sau đó chạy lại `npx prisma migrate deploy`. (c) Nếu **dở dang** (một phần SQL đã chạy): sửa tay DB cho khớp migration (hoặc hoàn tác thủ công rồi `--rolled-back`), **không** được `--applied` khi schema chưa khớp — dùng `prisma migrate diff` để đối chiếu. Sau khi resolve: redeploy / khởi động lại container `app`. Chi tiết: [Prisma migrate resolve](https://www.prisma.io/docs/guides/migrate/production-troubleshooting).
