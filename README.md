...
- Image gồm: build **frontend** (Vite) + **backend** (`tsc`, `prisma generate`). **`CMD`** (entrypoint): `npx prisma migrate deploy --schema=prisma/schema.prisma && node dist/src/server.js` — tức **mỗi lần container khởi động** chạy migration rồi mới chạy Node (mã nguồn trong `src/` → output dưới `dist/src/`). Nếu DB không kết nối được (ví dụ Prisma **P1001**), bước migrate sẽ lỗi và process thoát — cần **`DATABASE_URL`** trỏ đúng host Postgres **cùng mạng Docker** với app (tên service nội bộ, ví dụ dạng `...-postgres` / tên Dokploy tạo), Postgres đã **running** và cổng đúng. Cổng HTTP lắng nghe theo `PORT` (mặc định code fallback 5000 — nên set **`PORT=3000`** hoặc đúng cổng proxy của Dokploy).
