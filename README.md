...
| Chặn route theo quyền | `frontend/src/components/PermissionRoute.tsx` |
...
- `min_note_characters` (INTEGER, mặc định `10`): tối thiểu ký tự ghi chú lịch sử tác động.
...
- `POST /api/sales/interaction` — ghi nhận tương tác (body có thể có `detail`, `processingStatus`, `syncProcessingToDataPool`); độ dài tối thiểu theo `min_note_characters` (ưu tiên kiểm tra `detail` nếu có)
...
- Nguồn marketing:
  ...
  - chặn xóa nếu nguồn đang dùng bởi campaign.
- Campaign:
...
- Lead marketing:
  - `GET /api/marketing/leads` (lọc theo source/campaign/status/search/tagIds; áp dụng View Scope qua `buildCustomerWhereByScope`)
  - `POST /api/marketing/leads` (tạo lead thủ công): **bắt buộc** `phone` và `note` (ghi chú). Nguồn/chiến dịch/cây trồng và các trường khác **không** bắt buộc; nếu có chọn cây tính theo gốc thì vẫn validate số gốc. (FE: `MarketingCustomerForm`.)
  ...
  - Trường hợp SĐT trùng:
    - không tạo customer mới
    - ghi note + tạo lịch sử + tạo notifications cho đúng nhóm mục tiêu (dup logic trong `leadDuplicateService`).
- Chi phí campaign:
  - `GET/POST/PUT/DELETE /api/marketing/campaigns/:campaignId/costs`
...
- **Danh sách 66 quyền trong catalog** (10 nhóm, `backend/src/constants/permissionsCatalog.ts` — `DEFAULT_PERMISSIONS`):
  ...
  6. **Đơn hàng & Vận chuyển (6):** `VIEW_ORDERS`, `VIEW_ALL_COMPANY_ORDERS`, `CREATE_ORDER`, `MANAGE_ORDERS`, `MANAGE_SHIPPING`, `CREATE_ORDER_OUTSIDE_SYSTEM`
...
- Performance: `backend/src/routes/performanceRoutes.ts`
  - `GET /api/performance/marketing` — `VIEW_PERFORMANCE` hoặc `VIEW_REPORTS`
...
- Image gồm: build **frontend** (Vite) + **backend** (`tsc`, `prisma generate`), chạy `node dist/src/server.js` (mã nguồn trong `src/` → output dưới `dist/src/`); cổng lắng nghe theo `PORT` (mặc định code fallback 5000 — nên set **`PORT=3000`** hoặc đúng cổng proxy của Dokploy).
...
- **Sau các lần deploy có migration mới:** chạy lại **`npx prisma migrate deploy`**. Hiện `CMD` của image chỉ chạy server, không tự migrate (tránh fail khi DB chưa sẵn sàng).
